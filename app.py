        ...
        for page in doc:
            ...
            for img in images:
                ...
                if watermark_pattern is not None and img_bytes:
                    try:
                        # Convert user pattern to grayscale array (already done by caller)
                        # Use helper to remove pattern; returns PNG bytes
                        cleaned_bytes = remove_watermark_from_image(img_bytes, watermark_pattern)
                        # If cleaned_bytes differ from original, overlay onto each occurrence
                        if cleaned_bytes != img_bytes and rects:
                            for rect in rects:
                                try:
                                    page.insert_image(rect, stream=cleaned_bytes)
                                except Exception:
                                    continue
                    except Exception:
                        pass
