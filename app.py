...
# Immediately start the ping thread when the module is loaded.  The
# helper ensures the thread is started only once per process.  In
# production, each worker process will import this module and run
# `start_ping_thread()`, creating its own daemon thread.  Note that
# the older Flask decorator `before_first_request` was removed in
# Flask 2.3【472863986113245†L161-L176】; starting the thread here avoids using the
...
if __name__ == '__main__':
    ...
    # background ping thread explicitly.  The thread is normally started
    # at module import time so that WSGI servers (e.g. Gunicorn) also
    # create a ping thread for each worker.  Calling ``start_ping_thread``
    # again here is harmless because the helper ensures the thread is started
